...
if($action='logout'&&$uid&&$username)//退出系统
{
	...
	$DB->query('update '.$usertable." set `sessionid`='$sessionid' where uid='$uid'");
	ob_end_clean();
	ob_start();
...
if(RQ_POST)
{
	...
	$sql='Select * from '.$usertable." where `username`='$lusername'";
	...
	if($result)
	{
		if($result['password']==md5($lpassword))
		{
			...
			else
			{
				...
				$DB->query('update '.$usertable." set `logincount`=`logincount`+1,`loginip`='$onlineip',`logintime`='$timestamp',`sessionid`='$sessionid',`useragent`='$useragent' where uid='$uid'");
